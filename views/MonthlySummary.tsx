...
const MonthlySummary: React.FC<MonthlySummaryProps> = ({ 
  ...
  const getCaseTypeExplanation = (caseType?: string) => {
    switch (caseType) {
      case 'Falla Técnica CCTV':
        return 'Reportes vinculados a cámaras, DVR, grabación, monitoreo o fallas del sistema CCTV.';
      case 'Delito contra la Propiedad':
        return 'Reportes vinculados a hurto, robo o afectación patrimonial en la sede.';
      case 'Falla de Procedimiento':
        return 'Reportes vinculados a incumplimientos operativos, controles internos o protocolos de seguridad.';
      case 'Pendiente de Clasificar':
        return 'Casos registrados que todavía no tienen una clasificación suficientemente clara.';
      default:
        return 'Sin casos registrados para analizar en este período.';
    }
  };
    ...
    <div className="max-w-[1600px] mx-auto p-6 md:p-10 pb-20 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100 relative overflow-hidden group hover:scale-[1.02] transition-transform">
          ...
          <div className="flex items-end gap-3 relative z-10">
            <span className="text-5xl font-black text-slate-800 tracking-tighters">{coverage}%</span>
        ...
        <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl border border-slate-800 relative overflow-hidden">
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div className="flex items-start gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm transition-colors hover:bg-white/10">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Incidente más reportado en los casos</p>
                <p className="text-white font-bold leading-tight text-sm">
                  {topCaseType ? topCaseType[0] : "Sin casos registrados"}
                </p>
                <p className="text-xs text-blue-400 mt-1">
                  {topCaseType ? `${topCaseType[1]} caso(s). ${getCaseTypeExplanation(topCaseType[0])}` : "No hay casos suficientes para identificar una tendencia."}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm transition-colors hover:bg-white/10">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Casos activos de alta prioridad pendientes</p>
                <p className="text-white font-bold leading-tight text-sm">
                  {highPriorityOpen > 0 ? `${highPriorityOpen} caso(s) sin cierre` : "Sin casos críticos pendientes"}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  {highPriorityOpen > 0 ? "Corresponde a casos marcados con prioridad Alta que todavía no han sido cerrados." : "No existen casos de prioridad Alta pendientes por resolver."}
                </p>
              </div>
            </div>
