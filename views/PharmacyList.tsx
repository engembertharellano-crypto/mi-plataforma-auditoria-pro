...
const PharmacyList: React.FC<PharmacyListProps> = ({ 
  ...
  const handleEditClick = (pharmacy: Pharmacy) => {
    ...
    setFormData({ 
      ...
      operativa: pharmacy.operativa !== false
    });
  };
  ...
  const handleSaveNew = () => { 
    ...
    onAdd({ 
      ...
      operativa: newPharmacyData.operativa
    }); 
    ...
    <div className="max-w-7xl mx-auto p-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredPharmacies.length > 0 ? filteredPharmacies.map(pharmacy => (
          <div key={pharmacy.id} className={`glass-card rounded-[2rem] p-1 border border-white/60 shadow-lg hover:shadow-2xl transition-all duration-300 group hover:-translate-y-2 ${pharmacy.operativa === false ? 'opacity-80' : ''}`}>
            <div className="h-44 bg-slate-100 rounded-t-[1.8rem] rounded-b-xl relative overflow-hidden">
              {pharmacy.photo ? (
                <img src={pharmacy.photo} alt={pharmacy.name} className={`w-full h-full object-cover ${pharmacy.operativa === false ? 'grayscale contrast-75' : ''}`} />
              ) : (
              ...
              <div className="absolute bottom-3 left-3 flex flex-wrap gap-1">
                ...
                {pharmacy.operativa === false && (
                  <span className="bg-red-600 backdrop-blur text-white text-[9px] font-bold px-2 py-1 rounded-lg uppercase flex items-center gap-1 shadow-lg">
                    <PowerOff className="w-3 h-3" /> No Operativa
                  </span>
                )}
            ...
            <div className="p-6">
              <h3 className={`text-xl font-bold mb-1 leading-tight ${pharmacy.operativa === false ? 'text-slate-400' : 'text-slate-800'}`}>{pharmacy.name}</h3>
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <label className="block text-sm font-bold text-slate-700">Estado de Operación</label>
                  <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl bg-slate-50">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input type="checkbox" className="sr-only peer" checked={newPharmacyData.operativa} onChange={e => setNewPharmacyData({...newPharmacyData, operativa: e.target.checked})} />
                    ...
                    <span className="text-sm font-medium text-slate-600">{newPharmacyData.operativa ? "Operativa" : "No Operativa"}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl bg-slate-50">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input type="checkbox" className="sr-only peer" checked={formData.operativa} onChange={e => setFormData({...formData, operativa: e.target.checked})} />
                      <div className="w-11 h-6 bg-red-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-600"></div>
                    </label>
                    <span className="text-sm font-medium text-slate-600">{formData.operativa ? "Operativa" : "No Operativa"}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      {viewingPharmacy.operativa === false && (
                        <span className="bg-red-50 text-red-600 text-[10px] font-black px-2 py-1 rounded-lg border border-red-100 uppercase flex items-center gap-1">
                          <PowerOff className="w-3 h-3" /> No Operativa
                        </span>
                      )}
                    </div>
                  ...
                  <div className="space-y-8">
                    {viewingPharmacy.photo ? (
                      <div className="rounded-3xl overflow-hidden shadow-xl border-4 border-slate-50 bg-slate-100 aspect-video relative">
                        <img src={viewingPharmacy.photo} alt={viewingPharmacy.name} className={`w-full h-full object-cover ${viewingPharmacy.operativa === false ? 'grayscale contrast-75' : ''}`} />
                        <div className="absolute top-4 left-4 flex flex-col gap-2">
                          {viewingPharmacy.operativa === false && (
                             <span className="bg-red-600/90 backdrop-blur text-white text-xs font-black px-3 py-1.5 rounded-xl uppercase flex items-center gap-2 shadow-lg">
                               <PowerOff className="w-4 h-4" /> Local No Operativo
                             </span>
                          )}
                      ...
                      <div className="rounded-3xl border-4 border-dashed border-slate-100 aspect-video flex flex-col items-center justify-center text-slate-200 relative">
                        ...
                        <div className="absolute top-4 left-4 flex flex-col gap-2">
                          {viewingPharmacy.operativa === false && (
                             <span className="bg-red-50 text-red-600 text-xs font-black px-3 py-1.5 rounded-xl uppercase flex items-center gap-2 border border-red-100">
                               <PowerOff className="w-4 h-4" /> No Operativa
                             </span>
                          )}
                          {viewingPharmacy.hasSecurityOfficer ? (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-white border-2 border-slate-50 p-6 rounded-2xl shadow-sm flex flex-col">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Líder de Sede</p>
                        </div>
                        {manager ? (
                          <>
