...
const PharmacyList: React.FC<PharmacyListProps> = ({ pharmacies, staffRecords, onUpdate, onDelete, onAdd, currentUser }) => {
  ...
  const handleSaveNew = () => {
    ...
    setNewPharmacyData({ name: '', address: '', corporatePhone: '', zone: (isAdmin ? 'Gran Caracas Llanos' : currentUser?.zone) as Pharmacy['zone'], location: null, photo: null });
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="relative mb-8 group">
        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><Search className="w-5 h-5 text-slate-400 group-focus-within:text-orange-500" /></div>
        <input type="text" placeholder="Buscar farmacia..." className="w-full pl-12 pr-4 py-4 border border-white/20 rounded-2xl outline-none focus:ring-4 bg-white shadow-sm transition-all text-lg" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      </div>
          ...
          <div key={pharmacy.id} className="glass-card rounded-[2rem] p-1 border border-white/60 shadow-lg hover:shadow-2xl transition-all duration-300 group hover:-translate-y-2">
             <div className="h-44 bg-slate-100 rounded-t-[1.8rem] rounded-b-xl relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3 flex gap-2">
                   <button onClick={() => setViewingPharmacy(pharmacy)} className="p-2.5 bg-white/90 backdrop-blur-md rounded-xl text-slate-600 hover:text-blue-600 shadow-sm"><Eye className="w-4 h-4" /></button>
                   <button onClick={() => handleEditClick(pharmacy)} className="p-2.5 bg-white/90 backdrop-blur-md rounded-xl text-slate-600 hover:text-orange-600 shadow-sm"><Pencil className="w-4 h-4" /></button>
                </div>
                <div className="absolute bottom-3 left-3"><span className="bg-black/60 backdrop-blur text-white text-[9px] font-bold px-2 py-1 rounded-lg uppercase">{pharmacy.zone}</span></div>
             </div>
             <div className="p-6">
               ...
               <div className="flex items-center justify-between pt-4 border-t border-slate-100/50">
                  <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wide border shadow-sm ${pharmacy.risk === 'Bajo' ? 'bg-emerald-50 text-emerald-600' : pharmacy.risk === 'Medio' ? 'bg-orange-50 text-orange-600' : 'bg-red-50 text-red-600'}`}>{pharmacy.risk ? `Riesgo ${pharmacy.risk}` : 'Sin Evaluar'}</span>
                  <button onClick={() => setDeleteConfirmation({ id: pharmacy.id, name: pharmacy.name })} className="text-slate-300 hover:text-red-500 rounded-lg p-2 transition-all"><Trash2 className="w-5 h-5" /></button>
               </div>
             </div>
          </div>
        ))}
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl animate-in fade-in zoom-in duration-200 overflow-hidden border border-white/20">
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-end gap-2 text-slate-400 text-xs italic pb-3">Dato opcional pero recomendado</div>
              </div>
              <div><label className="block text-sm font-bold text-slate-700 mb-2">Dirección</label><textarea rows={2} className="w-full p-3 border border-slate-200 rounded-xl outline-none resize-none" value={newPharmacyData.address} onChange={e => setNewPharmacyData({...newPharmacyData, address: e.target.value})} /></div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl animate-in fade-in zoom-in duration-200 overflow-hidden border border-white/20">
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div><label className="block text-sm font-bold text-slate-700 mb-2">Teléfono Corporativo</label><input type="tel" className="w-full p-3 border border-slate-200 rounded-xl outline-none" value={formData.corporatePhone} onChange={e => setFormData({...formData, corporatePhone: e.target.value})} /></div>
              <div><label className="block text-sm font-bold text-slate-700 mb-2">Dirección</label><textarea rows={2} className="w-full p-3 border border-slate-200 rounded-xl outline-none resize-none" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} /></div>
      ...
      {viewingPharmacy && (() => {
        // Cálculo quirúrgico para Gerente y Fuerza de Trabajo
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-8">
                    {viewingPharmacy.photo ? (
                      <div className="rounded-3xl overflow-hidden shadow-xl border-4 border-slate-50 bg-slate-100 aspect-video">
                        <img src={viewingPharmacy.photo} alt={viewingPharmacy.name} className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="rounded-3xl border-4 border-dashed border-slate-100 aspect-video flex flex-col items-center justify-center text-slate-200">
                  ...
                  <div className="space-y-8">
                    <div className="grid grid-cols-2 gap-4">
                      {/* MODIFICACIÓN 1: GERENTE DE SEDE */}
                      ...
                      {/* MODIFICACIÓN 2: FUERZA DE TRABAJO */}
                      <div className="bg-white border-2 border-slate-50 p-6 rounded-2xl shadow-sm flex flex-col">
                        <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="bg-orange-50/50 p-6 rounded-2xl border-2 border-orange-100/50">
                      ...
                      <p className="text-[11px] text-orange-700/70 font-medium leading-relaxed">Utilice esta ficha para validar datos en campo. En caso de discrepancias en la dirección o contacto, solicite actualización al Administrador Global.</p>
                    </div>
                  </div>
                </div>
              </div>
