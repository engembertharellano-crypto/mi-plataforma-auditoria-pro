...
const AuditResults: React.FC<AuditResultsProps> = ({ audit, onBack, onSaveReport }) => {
  ...
  const generateExecutiveReport = async (auditData: AuditState, stats: any) => {
    ...
    try {
      ...
      const prompt = `Genera un informe ejecutivo formal y directo. Auditor: ${auditorName}, Sede: ${pharmacyName}, Gerente: ${managerName}, Cumplimiento: ${stats.finalScore.toFixed(2)}%, Riesgo: ${stats.riskLevel.toUpperCase()}. Hallazgos: ${failures.join(', ')}. ${vaultIncidentText} Estilo técnico, profesional.`;
      ...
      if (e.message?.includes('quota')) {
        ...
        setReportText("Error al conectar con el servidor de inteligencia estratégica.");
      }
    } finally {
  ...
  useEffect(() => {
    if (calculatedData && !audit.reportText && !isGenerating && !hasGenerated.current) {
        ...
        <div className="animate-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="bg-white rounded-3xl shadow-3xl border border-white p-8">
            ...
            <div className="bg-slate-50/50 rounded-2xl p-6 border border-slate-100 relative">
              ...
              <textarea className="w-full min-h-[450px] bg-transparent outline-none text-slate-700 leading-relaxed font-mono text-sm resize-none" value={reportText} onChange={(e) => setReportText(e.target.value)} spellCheck={false} />
            </div>
          </div>
        </div>
      ) : (
